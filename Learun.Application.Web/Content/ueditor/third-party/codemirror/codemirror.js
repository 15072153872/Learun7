...
var CodeMirror = (function() {
    ...
    function CodeMirror(place, givenOptions) {
        ...
        TextMarker.prototype.clear = operation(function() {
            ...
            for (var i = 0, e = this.set.length; i < e; ++i) {
                var line = this.set[i], lr = line.marked;
                if (!lr || !line.parent) continue;
                var lineN = lineNo(line);
                min = Math.min(min, lineN); max = Math.max(max, lineN);
                for (var j = 0; j < lr.length; ++j)
                    if (lr[j].set == this.set) lr.splice(j--, 1);
            }
            if (min != Infinity)
    ...
    Line.inheritMarks = function(text, orig) {
        var ln = new Line(text), lr = orig && orig.marked;
        if (lr) {
            for (var i = 0; i < lr.length; ++i) {
                if (lr[i].to == null && lr[i].style) {
                    var newlr = ln.marked || (ln.marked = []), mark = lr[i];
                    var nmark = mark.dup(); newlr.push(nmark); nmark.attach(ln);
                }
            }
        }
        return ln;
    ...
    Line.prototype = {
        ...
        replace: function(from, to_, text) {
            var st = [], lr = this.marked, to = to_ == null ? this.text.length : to_;
            ...
            if (lr) {
                ...
                for (var i = 0, mark = lr[i]; i < lr.length; ++i) {
                    mark.clipTo(from == null, from || 0, to_ == null, to, diff);
                    if (mark.isDead()) {mark.detach(this); lr.splice(i--, 1);}
                }
            }
        },
        // Split a part off a line, keeping styles and markers intact.
        split: function(pos, textBefore) {
            var st = [textBefore, null], lr = this.marked;
        ...
        append: function(line) {
            var mylen = this.text.length, lr = line.marked, mylr = this.marked;
            this.text += line.text;
            copyStyles(0, line.text.length, line.styles, this.styles);
            if (mylr) {
                for (var i = 0; i < mylr.length; ++i)
                    if (mylr[i].to == null) mylr[i].to = mylen;
            }
            if (lr && lr.length) {
                if (!mylr) this.marked = mylr = [];
                outer: for (var i = 0; i < lr.length; ++i) {
                    var mark = lr[i];
                    if (!mark.from) {
                        for (var j = 0; j < mylr.length; ++j) {
                            var mymark = mylr[j];
        ...
        fixMarkEnds: function(other) {
            var lr = this.marked, olr = other.marked;
            if (!lr) return;
            for (var i = 0; i < lr.length; ++i) {
                var mark = lr[i], close = mark.to == null;
                if (close && olr) {
                    for (var j = 0; j < olr.length; ++j)
                        if (olr[j].sameSet(mark)) {close = false; break;}
                }
