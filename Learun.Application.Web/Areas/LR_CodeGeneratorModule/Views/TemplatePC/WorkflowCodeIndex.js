                ...
                if (data.direction == "next") {
                    if (data.step == 1) {
                        ...
                        if (mapField[databaseLinkId + mainTable.name]) {
                            ...
                            $('#workfield').lrselectRefresh({ data: mapField[databaseLinkId + mainTable.name] });
                        }
                        else {
                            if (!mapField[databaseLinkId + mainTable.name]) {
                                learun.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DatabaseTable/GetFieldList', { databaseLinkId: databaseLinkId, tableName: mainTable.name }, function (data) {
                                    ...
                                    $('#workfield').lrselectRefresh({ data: data });
                                });
                            }
                        }
                        ...
                        var workfield = $('#workfield').lrselectGet();
                        if (!workfield) {
                            learun.alert.error('请选择流程关联字段！');
                            return false;
                        }
                    }
                    else if (data.step == 5) {
