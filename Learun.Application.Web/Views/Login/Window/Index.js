...
(function ($) {
    ...
    var lrPage = {
        init: function () {
            ...
            $('body').css('background', '#F8F8F8 url(' + $.rootUrl + '/Content/images/Login/windowbg.png) no-repeat');
            $('.lr-login-logo').css('background', 'url(' + $.rootUrl + '/Content/images/Login/windos_left.png) no-repeat');
            $('#psw_change').css('background', 'url(' + $.rootUrl + '/Content/images/Login/psw0.png) no-repeat center center');
        ...
        logining: function (isShow) {
            if (isShow) {
                ...
                $("#lr_login_btn").css('background','#156EDB');
            }
        },
        tip: function (msg) {
            var $tip = $('.lr-login-warning');
            $tip.hide();
            if (!!msg) {
                $tip.find('span').html(msg);
                $('.lr-login-head-sub').hide();
                $tip.show();
            }
        }
    };
